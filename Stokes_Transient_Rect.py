...
while batch_counter < 333:
    ...
    # obs = obs_gen.get_random_triangle()
    obs,v1 = obs_gen.get_random_rectangle()
    Char_Len = v1
    
    uo = .05
    Re = 4000*np.random.rand(1)[0]

    la = 1. # velocity of the scheme
    rhoo = 1. # fluid density
    mu = rhoo*uo*Char_Len/Re # bulk visocity
    # print(mu)
    ...
    if numpy_save:
        ...
        path =  os.getcwd() + '\\..\\data\\Rectangle_Trans_30fps\\'
        filename = 'Rectangle_Trans_' + str(month) + '_' + str(day) + '_' + str(year) + '_' + str(batch_counter)+ '_' + str(int(Re))
        ...
        image = ax.image(vorticity(sol), cmap='plasma', clim=[0, 0.1])
        epsilon = 1e-3
        prev_x = sol.m[qx]
        prev_y = sol.m[qy]
        ...
        # run the simulation
        fig.animate(update, interval=1/30)
        fig.show()
