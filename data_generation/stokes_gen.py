...
for obs_num in range(n_points):
    ...
    dico = {
        ...
        'elements': [obs],#[pylbm.Circle([.5, 0.5*(ymin+ymax)+2*dx], radius, label=2)],
        'space_step': dx,
        'scheme_velocity': LA,
        'schemes': [
            {
    ...
    if numpy_save:
        ...
        filename = 'stokes_triangle_' + str(int(Re))
        path = './data_' + filename
        ...
        if stable:
            ...
            np.savez(filename,sdf = sdf_map, vx=vx_map, vy=vy_map, rho=rho_map)
        else:
